...
const quickJumps = computed(() => {
  ...
  if (props.measureCount > 8) jumps.push({ label: 'Â¼', bar: Math.floor(props.measureCount / 4) })
  if (props.measureCount > 4) jumps.push({ label: 'Â½', bar: Math.floor(props.measureCount / 2) })
  if (props.measureCount > 8) jumps.push({ label: 'Â¾', bar: Math.floor((props.measureCount * 3) / 4) })
